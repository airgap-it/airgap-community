...
import { BitcoinProtocolNetwork, BitcoinProtocolOptions, BitcoinStandardProtocolNetwork, BitcoinUnits, ProtocolIdentifier } from '../types/protocol'
...
export class BitcoinProtocolImpl implements BitcoinProtocol {
  ...
  public readonly units: ProtocolUnitsMetadata<BitcoinUnits> = {
    BTC: {
      symbol: { value: 'DEVBTC', market: 'btc' },
      decimals: 8
    },
    mBTC: {
      symbol: { value: 'mDEVBTC' },
      decimals: 4
    },
    Satoshi: {
      symbol: { value: 'DEVSatoshi' },
      decimals: 0
    }
  }
  ...
  public readonly metadata: ProtocolMetadata<BitcoinUnits> = {
    identifier: ProtocolIdentifier.BTC,
    name: 'Bitcoin (ISO Example)',
  ...
  public async getBalanceOfAddresses(addresses: string[]): Promise<Balance<BitcoinUnits>> {
    ...
    return {
      total: newAmount(valueAccumulator.toString(), 'blockchain')
    }
  }
  ...
  public async getTransactionFeeWithPublicKey(
    ...
    return {
      low: newAmount(lowFee.toString(), 'blockchain'),
      medium: newAmount(mediumFee.toString(), 'blockchain'),
      high: newAmount(highFee.toString(), 'blockchain')
    }
  }
  ...
  private async prepareTransactionWithExtendedPublicKey(
    ...
    for (const utxo of utxos) {
      ...
      if (derivedAddress === utxo.address) {
        ...
        throw new Error(`Invalid address ${JSON.stringify(utxo.address)} returned from API`)
      }
